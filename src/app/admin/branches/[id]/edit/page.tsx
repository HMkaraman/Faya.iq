...
export default function EditBranchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [branchRes, catRes, teamRes] = await Promise.all([
        ...
        setCityEn(branch.city?.en || "");
        setCityAr(branch.city?.ar || "");
        setAddressEn(branch.address?.en || "");
        setAddressAr(branch.address?.ar || "");
        setPhone(branch.phone || "");
        setWhatsapp(branch.whatsapp || "");
        setEmail(branch.email || "");
        setHoursEn(branch.hours?.en || "");
        setHoursAr(branch.hours?.ar || "");
        setRating(branch.rating || 0);
        setReviewCount(branch.reviewCount || 0);
        setImage(branch.image || "");
        setMapUrl(branch.mapUrl || "");
        setLat(branch.coordinates?.lat || 0);
        setLng(branch.coordinates?.lng || 0);
        setAvailableServices(branch.availableServices || []);
        setSelectedTeam(branch.teamMembers || []);

        // Parse features into rows
        const enFeatures: string[] = branch.features?.en || [];
        const arFeatures: string[] = branch.features?.ar || [];
        const maxLen = Math.max(enFeatures.length, arFeatures.length, 1);
        ...
        toast("Failed to load branch data", "error");
      } finally {
        setLoading(false);
      }
    }
  ...
  function toggleTeamMember(memberId: string) {
    setSelectedTeam((prev) =>
      prev.includes(memberId) ? prev.filter((t) => t !== memberId) : [...prev, memberId]
    );
  }
