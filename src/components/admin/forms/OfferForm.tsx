  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <BilingualTextarea label="Description" nameEn="desc_en" nameAr="desc_ar" valueEn={form.description.en} valueAr={form.description.ar} onChangeEn={(v) => updateForm("description", { ...form.description, en: v })} onChangeAr={(v) => updateForm("description", { ...form.description, ar: v })} />
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <FormField label="Discount">
          <input type="text" value={form.discount} onChange={(e) => updateForm("discount", e.target.value)} placeholder="e.g. 30%, Buy 1 Get 1" className={inputClass} />
        </FormField>
        <BilingualInput label="Original Price" nameEn="originalPrice_en" nameAr="originalPrice_ar" valueEn={form.originalPrice.en} valueAr={form.originalPrice.ar} onChangeEn={(v) => updateForm("originalPrice", { ...form.originalPrice, en: v })} onChangeAr={(v) => updateForm("originalPrice", { ...form.originalPrice, ar: v })} />
        <BilingualInput label="Sale Price" nameEn="salePrice_en" nameAr="salePrice_ar" valueEn={form.salePrice.en} valueAr={form.salePrice.ar} onChangeEn={(v) => updateForm("salePrice", { ...form.salePrice, en: v })} onChangeAr={(v) => updateForm("salePrice", { ...form.salePrice, ar: v })} />
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <BilingualInput label="Branches" nameEn="branches_en" nameAr="branches_ar" valueEn={form.branches.en} valueAr={form.branches.ar} onChangeEn={(v) => updateForm("branches", { ...form.branches, en: v })} onChangeAr={(v) => updateForm("branches", { ...form.branches, ar: v })} />
        <BilingualInput label="Tag" nameEn="tag_en" nameAr="tag_ar" valueEn={form.tag.en} valueAr={form.tag.ar} onChangeEn={(v) => updateForm("tag", { ...form.tag, en: v })} onChangeAr={(v) => updateForm("tag", { ...form.tag, ar: v })} />
      ...
      <div className="flex items-center justify-end gap-3 pb-6">
        <Link href="/admin/offers" className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Cancel</Link>
        <button type="submit" disabled={isSubmitting} className="inline-flex items-center gap-2 px-6 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          ...
          {isSubmitting ? (isEditing ? "Saving..." : "Creating...") : (isEditing ? "Save Changes" : "Create Offer")}
        </button>
      </div>
    </form>
  );
